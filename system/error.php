...
class error {
  ...
  */
  public function trigger_error($error_message = NULL, $error_type = 'Undefined') {
    if ('dev' === __PROJECT_ENVIRONMENT) {
      trigger_error('<fieldset class="system_alert"><legend>' . $error_type . ' Error</legend><pre>' . htmlentities($error_message, ENT_QUOTES, 'UTF-8', false) . '<hr /><h4>PHP Backtrace</h4>' . print_r(debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS), true) . '<hr /><h4>PHP Functions Backtrace</h4>' . $this->debug_string_backtrace() . '</pre></fieldset>');
      return true;
    } else {
  ...
  */
  public function email_error($error_message, $error_type) {
    ...
    $email_body = '<fieldset class="system_alert"><legend>' . $error_type . ' Error</legend><pre>' . htmlentities($error_message, ENT_QUOTES, 'UTF-8', false) . '<hr /><h4>PHP Backtrace</h4>' . print_r(debug_backtrace(DEBUG_BACKTRACE_IGNORE_ARGS), true) . '<hr /><h4>PHP Functions Backtrace</h4>' . $this->debug_string_backtrace() . '</pre></fieldset>';
